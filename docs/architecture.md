...
| `0x18`–`0x1B` | `MOD0..3_CFG` | R/W | `[mode:4][keycode:4]` — any key can be a modifier |
...
| `0x1A…` | `LEDn_RGB` | R/W | Per-LED colour, 3 bytes (v2, APA102) |
...
[`modules/smartiny-key/docs/ladder.md`](../modules/smartiny-key/docs/ladder.md) (generated by
`modules/smartiny-key/tools/ladder_optimizer.py`). Recommended: Rr = [0, 5.6 k, 11 k, 16 k],
...
- **Single-key only** — a second press adds a *parallel* path, so `R_total` falls
  ...
  **latched/sticky modifiers make the restriction a non-issue**: the user never
  ...
  full table: `modules/smartiny-key/docs/ladder.md` § Multi-press behaviour.
...
10. ✅ **DONE — resistor-ladder values locked** (see `modules/smartiny-key/docs/ladder.md`).
    Remaining sub-choice: unified PCINT wake (recommended) vs decoupled wake for
    the extra 3 counts of resolution.
11. ✅ **DONE — `WHO_AM_I` id space assigned** in `lib/smartiny-common/smartiny_regs.h`
    (key `0x01`, led `0x02`, pwr `0x03`, mem `0x04`, calc `0x05`; `0x00`/`0xFF`
    reserved invalid so a stuck-low or absent bus cannot look like a module).
